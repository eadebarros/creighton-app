...
| (qualquer) | NONE + reflexo de luz* | — | `4` | Fértil (alerta) |
...
`*` "Reflexo de luz" (código 4/Shiny) não tem campo próprio no schema atual — **RISCO SINALIZADO**: o briefing menciona esse biomarcador na tabela VDRS (2.1) mas não no schema de `DAILY_ENTRY` (3.1). Isso precisa de um `DECISÃO DE EDU`: ou adicionamos um campo booleano `shiny_reflex` na tela de captura, ou aceitamos que ele fica coberto pela categoria WET/DAMP com stretch NONE (perda de granularidade clínica). Ver Seção 8, pergunta 1.
...
**RISCO SINALIZADO:** o briefing (2.2) diz "retorno ao padrão seco ou não-pico por pelo menos 24 horas consecutivas" mas depois o fluxograma (2.2, parte inferior) fala em confirmar após D+1,D+2,D+3 (3 dias). Isso é uma inconsistência real no briefing — a regra clínica padrão do Creighton usa **3 dias de padrão não-pico** para confirmar retrospectivamente. Implementei a versão de 3 dias porque é a que aparece no fluxograma detalhado e é a regra clínica padrão publicada do método. **DECISÃO DE EDU:** confirmar com a fonte/instrutora se há caso de uso para confirmação em 24h (isso mudaria o texto da UI e o teste automatizado). Ver Seção 8, pergunta 2.
...
- **RISCO SINALIZADO:** essa regra (8–16 dias) só pode ser aplicada retroativamente, ou seja, o app vai ficar "sem saber" se um Tc é o Ápice real por até 16 dias depois dele. Isso é clinicamente correto mas tem impacto de UX pesado (usuária pode ficar 2+ semanas sem confirmação em modo pré-menopausa) — vale alinhar expectativa de UX com Edu antes de desenhar a tela desse modo. Ver Seção 8, pergunta 3.
...
- Ausência de campo `shiny_reflex` (biomarcador código 4) até resolução da pergunta 1 — log explícito de gap conhecido, não esquecimento.
- E2E encryption fica em Horizonte 2/3 dependendo da resposta de Edu na pergunta 4 — se a resposta for "MVP não precisa disso ainda", isso deve ser documentado como débito técnico aceito, não decisão silenciosa.

---

## 8. DECISÕES PENDENTES DE EDU (bloqueiam início de sprints específicas)

1. **Biomarcador "Shiny" (código 4):** adicionar campo próprio na captura (`shiny_reflex: boolean`) ou aceitar perda de granularidade nesse caso raro? — Bloqueia: finalizar schema de Sprint 0/1.
2. **Confirmação de Ápice: 3 dias (regra clínica padrão, implementada) ou existe caso de uso real para 24h como o texto do briefing sugere em um trecho?** — Bloqueia: teste de aceite do motor de regras (Sprint 0).
3. **UX do modo pré-menopausa com confirmação de Ápice podendo levar até 16 dias:** aceitar a tela "aguardando confirmação" por esse período, ou isso precisa de uma comunicação de produto diferente? — Não bloqueia MVP (Horizonte 2), mas vale decidir a direção agora para não redesenhar depois.
...
6. **Dashboard do parceiro (Seção 9.1):** expor os 4 tokens de cor completos ou manter a simplificação Verde/Branco do briefing original, tratando Amarelo e Vermelho como casos derivados? — Bloqueia: componente de UI da Sprint 3, mas vale decidir cedo porque afeta o contrato de dados que o backend expõe pro dashboard.
7. **Biomarcador Brilhante no fluxo de captura (Seção 9.2):** adicionar 5ª tela condicional ou aceitar que o MVP não distingue código `4` de `0`/`2`? — Mesma decisão da pergunta 1, mas agora do ponto de vista de UX, não só de schema.
...
**RISCO SINALIZADO:** o briefing (4.3, Tela do Homem) simplifica a comunicação para só "Status: Verde" e "Status: Branco" no dashboard do parceiro — mas o sistema de selos real tem 4 estados, incluindo Amarelo (lactação) e Vermelho (sangramento). Se o dashboard do Homem só reconhecer Verde/Branco, um casal em modo Lactação vai ver um estado não mapeado. **Decisão de Edu:** o dashboard do parceiro deve expor os 4 tokens completos, ou a simplificação para 2 estados (com Amarelo/Vermelho tratados como variação de um dos dois) é intencional? Isso é Sprint 3, mas define o componente de UI desde já.
...
2. Sensação (Seco / Úmido / Molhado / Lubrificante)
...
**Nota de implementação ligada à Seção 3.1:** a Tela 3 do briefing não tem pergunta própria para o biomarcador "Brilhante" (código 4, `shiny_reflex`) — reforça a Pergunta 1 da Seção 8: sem uma pergunta dedicada no fluxo, esse biomarcador não tem como ser capturado, então ou ele fica de fora do MVP por decisão consciente, ou entra uma 5ª pergunta condicional ("o papel refletiu luz?") quando sensação = Seco/Úmido. Isso muda o fluxo de "sempre 4 telas" para "4 ou 5 telas condicionais" — pequeno, mas o Coder não deve resolver isso sozinho.
