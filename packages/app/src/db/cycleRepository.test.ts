...
describe('cycleRepository', () => {
  ...
  it('createCycle then getActiveCycle reports hasEntries=false for a fresh cycle', async () => {
    const db = createTestSqlExecutor();
    await migrate(db);
    const id = await createCycle(db, '2026-01-01', testNewId, 'REGULAR');
  ...
  it('getCycleVariantMode reports the variant a cycle was actually created under', async () => {
    const db = createTestSqlExecutor();
    await migrate(db);
    const id = await createCycle(db, '2026-01-01', testNewId, 'LACTATION');
    expect(await getCycleVariantMode(db, id)).toBe('LACTATION');
  });
});
