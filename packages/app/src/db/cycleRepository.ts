 ...
 */
export async function createCycle(db: SqlExecutor, startDate: string, newId: () => string): Promise<string> {
  const id = newId();
  await db.runAsync(
    "INSERT INTO cycles (id, start_date, is_active, variant_mode_snapshot) VALUES (?, ?, 1, 'REGULAR')",
    [id, startDate],
  );
  return id;
}
